(function ($, window, undefined) {
  ...
  $.Calendario.prototype = {
    _init: function (options) {
      ...
      if (
        parseFloat($().jquery) >= 1.9 &&
        this.options.events.indexOf("hover") != -1
      )
        this.logError(
          "'hover' psuedo-name is not supported" +
            " in jQuery 1.9+. Use 'mouseenter' 'mouseleave' events instead."
        );
      this.options.events = this.options.events.split(",");
      this.options.zone =
        this.options.zone.charAt(0) != "+" && this.options.zone.charAt(0) != "-"
          ? "+" + this.options.zone
          : this.options.zone;
    ...
    _processCaldataObj: function (val, key) {
      ...
      if (val.startTime && !val.endTime)
        val.endTime =
          parseInt(val.startTime.split(":")[0]) +
          1 +
          ":" +
      ...
      if (/^\d{2}-DD-\d{4}/.test(key) || /^\d{2}-DD-YYYY/.test(key)) {
        var det =
            /^(\d{2})-DD-(\d{4})/.exec(key) || /^(\d{2})-DD-YYYY/.exec(key),
          chkDate;
    ...
    _processCaldata: function (caldata) {
      ...
      $.each(caldata, function (key, val) {
        ...
        } else self.logError(key + " is an Invalid Date. Date should not contain spaces, should be separated by '-' and should be in the " + "format 'MM-DD-YYYY'. That ain't that difficult!");
    ...
    _propDate: function ($cell, event) {
      var idx = $cell.index(),
        ...
        dateProp = {
          ...
          month: this.month + 1,
          monthname: this.options.displayMonthAbbr
            ? this.options.monthabbrs[this.month]
            : this.options.months[this.month],
          year: this.year,
          weekday: idx + this.options.startIn,
          weekdayname:
            this.options.weeks[idx == 6 ? 0 : idx + this.options.startIn],
        };
      $cell
        ...
        .each(function (i, e) {
          ...
          data.startTime[i] = new Date(
            $html.find("time.fc-starttime").attr("datetime")
          );
          data.endTime[i] = new Date(
            $html.find("time.fc-endtime").attr("datetime")
          );
          data.allDay[i] =
            $html.find("time.fc-allday").attr("datetime") === "true"
              ? true
              : false;
    ...
    _initEvents: function () {
      ...
      for (var i = 0; i < self.options.events.length; i++) {
        ...
        calendarioEventNameFormat[i] =
          "onDay" + event[i].charAt(0).toUpperCase() + event[i].slice(1);
        if (this.options[calendarioEventNameFormat[i]] === undefined)
          this.options[calendarioEventNameFormat[i]] = function (
            $el,
            $content,
            dateProperties
          ) {
        ...
        this.$el.on(event[i] + ".calendario", "div.fc-row > div", function (e) {
          ...
          self._propDate(
            $(this),
            calendarioEventNameFormat[$.inArray(e.type, event)]
          );
        });
      }
      ...
      this.$el
        ...
        .queue(function () {
          self.today = new Date();
          if (
            self.today.getMonth() == self.month ||
            self.today.getMonth() + 1 == self.month
          )
    ...
    _checkUpdate: function () {
      ...
      $.getScript("js/cal-update.js")
        .done(function (script, textStatus) {
          if (
            calendario.current != self.version() &&
            parseFloat(calendario.current) >= parseFloat(self.version())
          )
    ...
    _toDateTime: function (time, day, start) {
      var zoneH = parseInt(this.options.zone.split(":")[0]),
        zoneM = parseInt(
          this.options.zone.charAt(0) + this.options.zone.split(":")[1]
        ),
      ...
      if (start) {
        ...
        if (
          d.getTime() -
            new Date(
              Date.UTC(this.year, this.month, day, hStart, mStart, 0, 0)
            ).getTime() <
          0
        )
          d = new Date(
            Date.UTC(this.year, this.month, day + 1, hour, minutes, 0, 0)
          );
      }
    ...
    _timeHtml: function (day, date) {
      ...
      content +=
        '<time class="fc-allday" datetime="' + day.allDay + '"></time>';
      content +=
        '<time class="fc-starttime" datetime="' +
        this._toDateTime(day.startTime, date) +
        '">' +
        day.startTime +
        "</time>";
      content +=
        '<time class="fc-endtime" datetime="' +
        this._toDateTime(day.endTime, date, day.startTime) +
        '">' +
        day.endTime +
        "</time>";
      return content;
    ...
    _convertDayArray: function (day, date) {
      ...
      return this._wrapDay(
        wrap_days.join('</div><div class="fc-calendar-event">')
      );
    },
    _getBody: function () {
      ...
      for (var i = 0; i < 7; i++) {
        for (var j = 0; j <= 6; j++) {
          ...
          if (this.options.fillEmpty && (j < p || i > 0)) {
            if (day > monthLength) {
              inner =
                '<span class="fc-date fc-emptydate">' +
                (day - monthLength) +
          ...
          if (day <= monthLength && (i > 0 || j >= p)) {
            ...
            var strdate =
              ...
              strdatemonth =
                "MM-" + (day < 10 ? "0" + day : day) + "-" + this.year,
              dayDataMonth = this.caldata[strdatemonth],
              strdatemonthyear =
                "MM" + "-" + (day < 10 ? "0" + day : day) + "-YYYY",
    ...
    getCell: function (day) {
      var row = Math.floor(
          (day + this.startingDay - this.options.startIn - 1) / 7
        ),
    ...
    feed: function (callback) {
      var self = this;
      $.post(self.options.feedParser, { dates: this.caldata }).always(function (
        data
      ) {
  ...
  $.fn.calendario = function (options) {
    ...
    if (typeof options === "string") {
      ...
      this.each(function () {
        if (!instance) {
          logError(
            "Cannot call methods on calendario prior to initialization; Attempted to call method '" +
              options +
              "'"
          );
          return;
